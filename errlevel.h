...
#define D_SHOW_PARMS         LOGLEV(2, 5, 0)   /* show all parameters on program initiation */

#define D_HANDSHAKE          LOGLEV(3, 6, 0)   /* show data & control channel handshakes */
#define D_GREMLIN            LOGLEV(3, 7, 0)   /* show simulated outage info from gremlin module */

#define D_TLS_DEBUG_LOW      LOGLEV(4, 6, 0)   /* low frequency info from tls_session routines */

#define D_COMP_LOW           LOGLEV(5, 8, 0)   /* show adaptive compression state changes */

#define D_SHOW_KEYS          LOGLEV(6, 9, 0)   /* show data channel encryption keys */

#define D_HANDSHAKE_VERBOSE  LOGLEV(7, 10, 0)  /* show detailed description of each handshake */
